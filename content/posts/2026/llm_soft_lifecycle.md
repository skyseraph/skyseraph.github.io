---
title: "LLM时代软件范式迁移：从 Prompt Engineering 到 Harness Engineering"
...
toc: true
draft: false
summary: "从 Prompt Engineering 到 Harness Engineering，系统阐述 LLM 软件开发的三层范式演进与核心工程原则。"
description: "2025-2026年，AI 软件开发正经历从「人写代码」到「人描述意图，AI 生成执行」的根本性范式迁移。本文系统分析 Prompt Engineering、Context Engineering、Harness Engineering 三层演进，结合 Anthropic、OpenAI、Google 等头部企业实践，阐述范式迁移对工程师角色与产品设计的深远影响。"
---
...
- [2. 三代范式演进：从 Software 1.0 到 Software 3.0](#2-三代范式演进从-software-10-到-software-30)
  - [Software 1.0：命令式逻辑时代](#software-10命令式逻辑时代)
  - [Software 2.0：神经网络时代](#software-20神经网络时代)
  - [Software 3.0：LLM 时代](#software-30llm-时代)
...
- [5. 第二层：Context Engineering — 系统设计的崛起](#5-第二层context-engineering--系统设计的崛起)
  - [概念的诞生](#概念的诞生)
  - [核心问题](#核心问题)
  - [Context Rot：最关键的工程约束](#context-rot最关键的工程约束)
  - [关键设计原则](#关键设计原则)
  - [一个关键的类比重申](#一个关键的类比重申)
- [6. 第三层：Harness Engineering — 基础设施时代](#6-第三层harness-engineering--基础设施时代)
  - [定义的清晰化](#定义的清晰化)
  - [Harness 的核心架构组件](#harness-的核心架构组件)
  - [Harness Engineering 的产业证据](#harness-engineering-的产业证据)
  - [自然语言 Harness：下一个前沿](#自然语言-harness下一个前沿)
- [7. 主流 AI 公司的范式实践](#7-主流-ai-公司的范式实践)
  - [Anthropic：从模型到生态的系统性布局](#anthropic从模型到生态的系统性布局)
  - [OpenAI：Harness Engineering 的自我证明](#openaiharness-engineering-的自我证明)
  - [Google：全栈战略与 Agent 标准化](#google全栈战略与-agent-标准化)
  - [其他值得关注的实践](#其他值得关注的实践)
- [8. 大咖观点汇总](#8-大咖观点汇总)
- [9. 个人深度思考：范式迁移的底层逻辑](#9-个人深度思考范式迁移的底层逻辑)
  - [9.1 这不是工具升级，而是抽象层次的跃迁](#91-这不是工具升级而是抽象层次的跃迁)
  - [9.2 "模型竞争"的护城河正在消失](#92-模型竞争的护城河正在消失)
  - [9.3 软件工程师角色的重新定义](#93-软件工程师角色的重新定义)
  - [9.4 "Vibe Coding"的误解与真相](#94-vibe-coding的误解与真相)
  - [9.5 "上下文"正在成为企业的核心资产](#95-上下文正在成为企业的核心资产)
- [10. 建设性建议：如何在范式迁移中站稳脚跟](#10-建设性建议如何在范式迁移中站稳脚跟)
  - [对于个人工程师](#对于个人工程师)
  - [对于团队和组织](#对于团队和组织)
  - [对于 AI 产品设计者](#对于-ai-产品设计者)
...
2025—2026年，一个清晰的信号在产业界不断扩大：软件的生产方式、运行方式、甚至定义方式，都正在经历一次根本性的重写。这不是渐进式的工具升级，而是类比 PC 革命、互联网革命量级的**范式迁移**。

**三位顶级人物的判断，从三个不同层面描绘了同一现实：**

- **Dario Amodei**（Anthropic CEO）：Anthropic 内部工程师已基本不再手写代码——"I have engineers within Anthropic who say, 'I don't write any code anymore. I just let the model write the code. I edit it. I do the things around it.'" 他进一步预测，AI 将在 6 至 12 个月内完成软件工程师所做的全部工作。^[Yahoo Finance, *Anthropic CEO Predicts AI Models Will Replace Software Engineers In 6-12 Months*, 2026-01]
- **Sam Altman**（OpenAI CEO）：一个人将可以指挥等效于一万人的认知劳动力，做到过去需要庞大组织才能完成的事情。^[Sam Altman on X / OpenAI Blog, 2025]
- **Andrej Karpathy**：LLM 是继 1970-80 年代个人计算机之后"下一个主要计算范式"，而目前人类开发出的，还不到这个新范式全部潜力的 10%。^[Andrej Karpathy, *2025 LLM Year in Review*, karpathy.bearblog.dev, 2025-12]

**核心结论：我们正在从"人写代码让机器执行"，过渡到"人描述意图，AI 生成并执行代码"。**
...
以深度学习为核心，程序不再被"写出来"，而是被"训练出来"。参数是权重，行为从数据中涌现。Tesla 用神经网络替换了整个 C++ 自动驾驶栈中大量模块，是这一范式的代表性案例。**问题：不透明、不可解释、部署困难。**

### Software 3.0：LLM 时代

**LLM 被英语编程。** Karpathy 明确表示："LLMs are a new kind of computer, and you program them *in English*."^[ibid.] 软件的表达层从符号逻辑、矩阵权重，跃迁为**自然语言意图**。

这一范式的关键特征是：**程序 = 意图描述 + 上下文信息 + 执行环境**。配合执行这一程序的"运行时"，就是 LLM 及其外部封装。

---

## 3. LLM 即操作系统：Karpathy 的计算机科学视角

Karpathy 在 2023 年最早系统性地提出了"LLM 即操作系统"的隐喻：
...
| 传统 OS 组件 | LLM OS 对应 |
|:------------|:------------|
| **CPU** | **LLM** — 核心推理单元，无状态的 token 预测器 |
| **RAM** | **Context Window** — 工作记忆，有限且宝贵 |
| **Operating System** | **Agent Harness** — 管理 I/O、内存调度、工具分发、安全策略 |
...
3. **它在 Agent 场景中失效。** 当 LLM 需要多轮执行、调用工具、处理中间结果时，"一次输入、一次输出"的假设已经完全不适用。
...
Gartner 在 2025 年旗帜鲜明地宣告：**"Context engineering is in, and prompt engineering is out."** ^[Gartner Research, 2025-07，转引自 IntuitionLabs.ai]
...
"Context Engineering" 在 2025 年 6 月正式引爆。Shopify CEO **Tobi Lütke** 率先定义：
...
它把整个"填充上下文窗口的系统"视为工程对象。填充上下文窗口的，不只是 prompt 指令，还包括：
...
**Context Rot（上下文腐烂）** 是 Context Engineering 的核心命题。

Stanford "Lost in the Middle" 系列研究证明：当关键信息被放置在长上下文的中间位置时，LLM 性能会显著下降。^[引用自 Firecrawl Blog, 2026-02]

Databricks 研究发现，LLM 准确率在 32K tokens 附近便开始明显下滑，远早于百万 token 上下文窗口的物理极限。
...
**核心结论：Context 是有限的且具有递减边际效应的资源，工程师必须像管理 CPU 缓存一样管理它。**

### 关键设计原则

**1. 最小必要信息原则（Minimal Signal Tokens）**

> "Not all information. The minimal set of high-signal tokens." ^[Firecrawl Blog, 2026-02]

应该删除所有与当前推理步骤无关的 token。
...
Context 不是一个静态模板，而是一个在每次 LLM 调用前动态运行的系统输出。维护轻量级指针（文件路径、查询语句），在运行时按需加载——这镜像了人类认知中的"检索而非背诵"机制。

**4. 层次化记忆管理（Memory Hierarchy）**

类比操作系统的内存层次（寄存器 → L1/L2 Cache → RAM → 磁盘），建立 AI 的记忆层次：

- **Working Memory**：当前上下文
- **Session Memory**：会话记录
- **Semantic Memory**：向量检索
- **Episodic Memory**：结构化事件存储
...
Thoughtworks Distinguished Engineer **Birgitta Böckeler** 给出了最早的系统性定义：Harness 是"除模型本身以外的一切"——系统提示、编排逻辑、工具调用、测试验证、反馈控制、安全约束…… ^[Birgitta Böckeler, *Harness Engineering for Coding Agent Users*, martinfowler.com, 2026-04]
...
根据 2026 年 4 月 Anthropic npm 包意外泄露事件揭示的 Claude Code 源码结构 ^[IDC Community, *The 3rd Generation of Agents: How "Harness Engineering" Changed Games Again*]，以及学术界的系统整理（Preprints.org 的 Agent Harness Survey, 2026-04），现代 Agent Harness 包含五层：

| Layer | 名称 | 核心职责 |
|:------|:-----|:--------|
| **1** | **执行循环（Execution Loop）** | 控制 Agent 的 Think → Act → Observe 循环，管理 token 预算，处理停止条件、超时、重试 |
| **2** | **工具层（Tool Layer）** | 管理工具注册、参数验证、执行代理（沙箱/容器）、结果格式化；MCP 协议在此层标准化工具接入 |
| **3** | **记忆与上下文层（Memory & Context）** | 实现多级记忆系统，处理 Context Rot，执行上下文压缩（Context Compaction）；Claude Code 的 "Ralph Loop" 模式在此层：当 Agent 因上下文焦虑试图提前退出时，将原始意图重注入压缩后的干净上下文窗口 |
| **4** | **安全与治理层（Safety & Governance）** | 拦截危险操作，执行权限策略（RBAC），审计日志；**Harness 是比模型权重更灵活的安全实施点** |
| **5** | **可观测性层（Observability）** | 轨迹记录、Token 消耗追踪、成功率监控、失败模式分析；闭环优化和 Data Flywheel 的基础 |

### Harness Engineering 的产业证据

- **TerminalBench 2.0**：仅仅修改 Harness（不换模型），可以让某个 Agent 从排名 30 名开外跃升至前 5。^[DecodingAI.com, 2026-04]
- **成本优化**：通过 Prefix Stability 和语义路由等 Harness 级别优化，Token 成本可从 **$3.00/MTok 降至 $0.30/MTok**，同时实现 4 倍延迟降低——无需换模型。^[Masood, ibid.]
- **ICML 2025 论文**：带 Harness 的模型在所有测试游戏中均优于无 Harness 的同款模型。^[Parallel.ai Blog, *What is an Agent Harness*]
- **OpenAI 最直接的内部案例**：一个三人团队通过 Harness Engineering，用 5 个月完成了约 **100 万行代码** 的仓库构建，平均每工程师每天 3.5 个 PR。^[OpenAI, *Harness Engineering: Leveraging Codex in an Agent-First World*, 2026-02]
...
2026 年 3 月，Tsinghua 团队与 Stanford/SambaNova/UC Berkeley 合作研究同期提出：**Natural-Language Agent Harnesses（NLAH）**——Harness 本身可以用自然语言而非 Python 代码来描述。^[arxiv.org/html/2603.25723v1]
...
Karpathy 将其评价为"the first convincing demonstration of what an LLM Agent looks like"。^[Karpathy, *2025 LLM Year in Review*]

Claude Code 的关键设计决策：**Local-first**（运行在开发者本机，而非云端），让 Agent 直接访问文件系统、环境变量、git 历史——这是 Harness 设计而非模型能力决定的。Anthropic 在 2026 年仅 Claude Code 一项的年化收入据估计已超过 **5 亿美元**。^[Turing College AI Engineering Guidebook, 2026]
...
Anthropic 工程团队于 2025 年正式发布博客 *Effective Context Engineering for AI Agents*，系统阐述了上下文腐烂、注意力预算、动态检索等核心概念。^[anthropic.com/engineering/effective-context-engineering-for-ai-agents]
...
Stripe 的 Minions 系统每周通过混合确定性节点（linters、CI）与 Agentic 循环的"Blueprint"架构自动提交超过 **1,000 个 PR**。^[Preprints.org, Agent Harness Survey, 2026-04]
...
Cursor 在 2026 年 3 月已突破年化 **20 亿美元**营收。这背后不是模型竞争，而是**上下文工程竞争**——Cursor 的差异化在于更好的代码库理解 Harness（@codebase 检索、多文件编辑协调等）。
...
| Andrej Karpathy | — | "Context engineering is the delicate art and science of filling the context window with just the right information." | X (@karpathy), 2025-06 |
| Andrej Karpathy | — | "Claude Code is 'the first convincing demonstration of what an LLM Agent looks like'." | 2025 LLM Year in Review, 2025-12 |
...
| Dario Amodei | Anthropic CEO | "We might be 6-12 months away from models doing all of what software engineers do end-to-end." | Multiple interviews, 2025-2026 |
| Sam Altman | OpenAI CEO | "10,000-person-equivalent companies run by one person." | Multiple statements, 2025 |
| Gartner | 研究机构 | "Context engineering is in, and prompt engineering is out." | Gartner Research, 2025-07 |
| Anthropic 工程团队 | Anthropic | "Context must be treated as a finite resource with diminishing marginal returns." | *Effective Context Engineering*, 2025 |
...
每次抽象提升都有相同的结构：**新的抽象层使大量原来必须显式处理的复杂性变得隐式**，但同时引入了新的隐式复杂性。

- SQL 让人不再关心 B 树的实现，但引入了查询优化器的黑盒问题
- LLM 让人不再关心函数实现，但引入了 Context 管理的新工程问题

Prompt Engineering → Context Engineering → Harness Engineering 的演进路径，正是沿着"管理新隐式复杂性"的方向展开的：**先学会说话，再学会管理信息，最后学会构建可靠系统。**

### 9.2 "模型竞争"的护城河正在消失

当底层模型能力趋于同质化（各大 LLM 在大多数基准测试上性能趋同），**差异化竞争将完全转移到 Harness 层**。
...
> "It's often said in AI product development now that 'the harness makes or breaks an AI product'. Two products might use the same underlying LLM, but the one with a superior harness will deliver a far better user experience." ^[Parallel.ai Blog]
...
Vibe Coding 降低了**写代码**的门槛，但提高了**系统设计**的重要性。一个没有基础的 Vibe Coder 可以很快生成"可以运行的代码"，但无法生成"可维护、可扩展、安全可靠的系统"。理解架构原则、数据模型、安全边界、性能约束——这些能力**变得更加核心**，因为 AI 不会自动考虑这些。

**真正的竞争力变成：能否精确地定义问题、约束空间、验证结果。** 这需要更深的领域理解，不是更少的。

### 9.5 "上下文"正在成为企业的核心资产

在 LLM 时代，数据的价值不再仅仅在于"拥有数据"，而在于**"将数据结构化为 Agent 可消费的上下文"**。

哈佛商学院 2026 年 2 月的研究指出：在每个公司都使用相同 AI 模型的前提下，**结构化的机构知识库**——记录决策过程、经验教训、业务规则——成为真正的差异化竞争优势。^[The Agent Arms Race Blog, 转引 HBR Research, 2026-02]
...
在你的 AI 系统能够无监督运行之前，先建立能够检验其输出的：

- **Computational Guide**：linters、类型检查器、单元测试
- **Inferential Sensor**：LLM-as-judge、语义相似度检查

让 AI 的结果可被快速验证，比让 AI 完全自主运行更有价值。^[Böckeler, martinfowler.com, 2026-04]

**③ 培养"意图精确性"的写作能力**

技术精确的自然语言写作——能够清晰描述任务约束、边界条件、验收标准——是 Software 3.0 时代**最核心的工程技能之一**。这比掌握某个框架更难学，也更难被替代。
...
一个 AI 产品的差异化，往往不在于用了哪个模型，而在于它能够为用户的任务提供**多丰富、多精准的上下文**。设计产品时，把"用户的知识、偏好、历史、工具"系统化地纳入 Context 管理框架，是比模型选型更重要的产品决策。
...
随着 AI Agent 成为新的主要信息消费者：

- CLI 优于 GUI（因为 CLI 输出更结构化）
- 结构化日志优于富文本界面
- API First 优于 UI First

Karpathy 明确指出：**LLMs are the new primary consumer of digital information**，产品设计应当为此做适配。^[Karpathy, AI Startup School Talk, 2025-06]

---

## 11. 结语：我们身处哪个时刻？

Karpathy 用了一个精确的历史坐标：**"We are computing circa ~1960s."** ^[ibid.]

上世纪 60 年代，计算机还是大型机，主要通过批处理、打卡输入来使用，只有少数专业人士能够接触。个人计算、图形界面、互联网，都还没有到来。

**我们现在所处的位置，大概就是那个时刻。**

今天的"对话框提示词"，是那个时代的命令行。Context Engineering 是我们正在学习如何分配有限的计算资源。Harness Engineering 是我们开始构建这个新计算范式的操作系统基础设施。

三层范式的迁移，不是一个结束，而是一个开始：
